...
\author{Kai Xu\textsuperscript{1}, Hong Ge\textsuperscript{2}, Will Tebbutt\textsuperscript{2}, Mohamed Tarek\textsuperscript{3} and Martin Trapp\textsuperscript{4}}
\institute{
...
\block{Abstract}{
    ...
    \texttt{AdvancedHMC.jl} (AHMC), a pure Julia implementation of Stan’s built-in NUTS algorithm and 
    related adaptation methods. 
    We hope \texttt{AdvancedHMC.jl} can help expose Stan’s NUTS to a wider range of users, 
...
\block{Hamiltonian Monte Carlo Components}{
    ...
    \texttt{AdvancedHMC.jl} supports the combination of various HMC samplers below.
    $$
    (\texttt{StaticHMC} \cup \texttt{DynamicHMC}) \times \texttt{Adaptor}.
    $$
    Here $\texttt{StaticHMC}$ are HMC with fixed-length trajectories and 
    $\texttt{DynamicHMC}$ are HMC with dynamic trajectories 
    ...
    \begin{equation*}
        ...
        \end{aligned}.
    \end{equation*}
    $\texttt{Adaptor}$ contains any compositional adaptor composed from base adaptors
    $$
    ...
    \textbf{Note 2}: a specific composition of base adaptors called $\texttt{StanHMCAdaptor}$ is provided to construct Stan's windowed adaptor, 
...
\block{Example Code of Building Stan's NUTS using AHMC}{
    \lstinputlisting{example.jl}
}

\block{Sampling Efficiency: Stan's NUTS v.s. AHMC}{
...
\block{Computational Efficiency: Stan v.s. Turing}{
    ...
    \begin{tikztable}[Time comparisons between \texttt{Stan} and \texttt{Turing} (AHMC) for five models using $^1$ 25 runs, $^2$ 50 runs or $^3$ 100 runs.]
    \scalebox{0.9}{
...
\block{Easy Integration of Other Julia Packages}{
    \textbf{Bijectors.jl} is used inside Turing.jl to do automatic transformations of constrained variables to run HMC.
    ...
    \textbf{SoSS.jl} is another PPL in Julia that
    uses AdvancedHMC.jl as its backend.
